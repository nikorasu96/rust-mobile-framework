...
| TD-005 | High | The example uses the committed pipeline, but legacy benchmark/tests and `Runtime::mount` still accept caller-assigned IDs | Migrate benchmark/contracts, then remove the bootstrap tree, renderer port and legacy mount use case before Android work |
