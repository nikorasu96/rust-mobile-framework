...
| TD-005 | High | Initial reconciliation owns `NodeId`, but the bootstrap runtime mount still accepts caller-assigned IDs | Integrate `PreparedCommit` into runtime, implement keyed updates, then remove the bootstrap tree before Android work |
