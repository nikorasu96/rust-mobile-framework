...
| TD-005 | High | Prepared commits now apply and promote through the runtime, but the bootstrap renderer still accepts caller-assigned IDs and partial-failure remount is not compiled | Add remount recovery, migrate the headless adapter, then remove the bootstrap tree before Android work |
