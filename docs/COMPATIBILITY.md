...
| Runtime mount port | Experimental | `rmf-runtime` legacy contract test | Caller-identified full-tree bootstrap remains only for the old example |
...
| Deterministic headless host | Rust implementation | atomic mutation/remount adapter and Rust contract tests | Not visual rendering; legacy example is not migrated |
